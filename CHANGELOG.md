...
- jsdom 验证：目录 14 条 / 0 失效锚点 / 折叠与点击跳转正常（hash 更新）；适配表 17 数据行 + 4 分组行 + 17 个展开按钮，分组顺序正确。SW en2-xxx。
...
- 全站 13 页导航加「作文模板」入口；SW 注册 writing.html/js/writing.js/data/writing_templates.json。jsdom 验证通过。SW en2-xxx。
...
- jsdom 数据校验：40 题解析齐全（text1-4 各 5 + cloze 20）、0 坏定位句、无过短解析。SW en2-xxx。
...
- jsdom 验证：两页星级元素 0、英二政法 13 卡年份降序、英一科学科技 18 卡、点全部恢复按年。SW en2-xxx。
...
- jsdom 验证：188 行 5 列 / 词频降序单调 / 原句最长 129 / 搜索 handsome 命中。SW en2-xxx。
...
- jsdom 验证：完形 6 段网格、新题型 6 段+5 空位卡整行、翻译 10 句网格、阅读 0 grid。SW en2-xxx。
