...
- jsdom 验证：主干 149 处单色（六色 0 残留）、选区高亮 store 1→点击取消归零→恢复 mark→清除全通过、词组弹卡/开关双向同步/清除按钮初始隐藏正常。SW en2-8185044e。
...
- jsdom 验证：the/a/and/while 均不可点、实词可点、词组 76 处完整、词组弹卡正常、文本无遗漏。SW en2-7336916b。
...
- jsdom 验证：染色 wr-subj 48 / wr-pred 58 / wr-obj 43 / trans 9、旧色块 0 残留；划词 684 词 + 词组命中 76 处（associated with / account for / stems from / pace of life…）；单词卡（词+音标+释义+生词本按钮+例句属性）、词组卡（整组释义+2 组成词行）、点空白关卡、开关双向同步（0↔48）、点词不点亮全部通过。SW en2-797a4300。
...
- jsdom 验证：40 行/中文行 40/功能句 15；染色 trunk 149 + lead 8 + trans 9 + caus 7，占位符高亮 54 处保留；点亮→中文联动→再点取消→重渲染恢复→清除全通过；开关关/开 0↔149；弹窗 8 条目英中对照、9 卡按钮齐全；中英 key 对齐。SW en2-1a77f792。
...
- jsdom 验证：完形 20 行卡/无徽标/空格联动绿色/解析折叠正常；新题型池 7 项浮层展开收起/胶囊 35 个（2025 带文字 35 个、2024 纯字母）/答对绿色+池闪烁；翻译无面板/参考卡正常/竖排无 sent-grid 残留。SW en2-3432ca11。
...
- jsdom 验证：文章页 2010 英文槽位高亮 15 / 中文槽位高亮 9 / 填槽表达 3 组 14 行 / 模板句型 8 条（{{}} 高亮 16 处）；写作页 17 篇合计英文槽位高亮 248、中文 232、填槽行 248、模板句型 143，TOC 14 条正常。SW en2-2e8a5e96。
...
- jsdom 验证：文章页 2010 示范 3 段 10 句、槽位高亮 14 / 自写高亮 9、面板展开收起正常；写作页 17 篇全部有面板
  （合计逐句行 169、槽位高亮 136、自写高亮 214），17 篇词数 165-177 全部落在 165-180 区间。SW en2-c8163cd1。
...
- jsdom 验证：短语页 6 类 263 条、写作句式 65 条 / 65 个跳转链接 / 8 个高频徽标 / 2 个变体行、搜索与多年份筛选正常；近义词页 2 张地图、原图 21 簇 101 条未受影响、写作句式 9 簇 65 条全部加粗渲染、点亮持久化正常。SW en2-d192fa72。
...
- jsdom 验证：目录 14 条 / 0 失效锚点 / 折叠与点击跳转正常（hash 更新）；适配表 17 数据行 + 4 分组行 + 17 个展开按钮，分组顺序正确。SW en2-c8163cd1。
...
- jsdom 验证：文章页套用示范区存在且顺序正确（示范在范文前）、题目译文显示、图表路径正确、中文切换正常；写作页表头 7 列、17 个展开按钮、17 行示范、2026 行数据完整。SW en2-a2fb430e。
...
- 全站 13 页导航加「作文模板」入口；SW 注册 writing.html/js/writing.js/data/writing_templates.json。jsdom 验证通过。SW en2-c8163cd1。
...
- jsdom 数据校验：40 题解析齐全（text1-4 各 5 + cloze 20）、0 坏定位句、无过短解析。SW en2-c8163cd1。
...
- jsdom 验证：两页星级元素 0、英二政法 13 卡年份降序、英一科学科技 18 卡、点全部恢复按年。SW en2-c8163cd1。
...
- jsdom 验证：两页 chip 6 个；英二点[政法]→13 卡（跨年、星级、年份 tab 隐藏）；点[全部]恢复 8 卡；英一点[科学科技]→18 卡全 en1_ 前缀、0 星级。SW en2-e819ee59。
...
- jsdom 验证：封面 3 卡配色 r-red/r-yellow/r-green ✓；统计页（真实题库数据）：tiles 5、错误率表首行=错误率最高文章、题型/年份表正常、qtype 画像「细节2/观点1」实时正确。SW en2-642a1a3d。
...
- jsdom 验证：单词 161 行（3 分区 / 19 年份 chip / 13 英文-only）；短语 5 卡片 / 198 条 / 17 年份 chip / 搜索命中。SW en2-fc6b0bb9。
...
- jsdom 验证：单词 161 行 / 3 分区 / 19 年份 chip / 13 条英文-only；短语 5 卡片 / 109 条 / 13 年份 chip。SW en2-a711c02f。
...
- jsdom 验证：122 行 / 3 分区 / 19 年份 chip（2026→2008）/ 12 条英文-only（英一与 2007-09 无可靠译文）/ 搜索命中。SW en2-e3abecea。
...
- jsdom 验证：三档分区 / 43 行 / 18 年份 chip / freq 列已删 / 搜索与年份筛选命中；短语 5 卡片 15 条 / 搜索命中。SW en2-6e7446e2（101 资源）。
...
- SW en2-e9d995d5；jsdom 验证 220 行 5 列 + 搜索命中。
...
- jsdom 验证：188 行 5 列 / 词频降序单调 / 原句最长 129 / 搜索 handsome 命中。SW en2-c8163cd1。
...
- jsdom 验证：21 卡 101 条/新簇/in that 加粗/点亮/旧高亮恢复全通过。SW en2-c8163cd1。
...
- jsdom 验证：14 簇 59 条目、点亮/取消/持久化/clear 全通过。SW en2-01367294。
...
- jsdom 验证：完形 6 段网格、新题型 6 段+5 空位卡整行、翻译 10 句网格、阅读 0 grid。SW en2-c8163cd1。

## [2026-09-07 优化] struct 结构树配色：主干改淡紫、整体调淡

- 主干(.st-t) 淡绿 → 淡紫（浅 #EDE8F9/#5B3E96，深 #3B2E63/#C9B8F0）；从句(.st-c) 与修饰(.st-m) 一并调淡但文字保持对比度。
- jsdom 验证结构树三类渲染与 sig-lead 引导词高亮正常。SW en2-c8163cd1。
...
- jsdom 验证：提示条创建/文本/防重载单条均通过。SW en2-1a1d423d。
